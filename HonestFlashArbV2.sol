...
contract HonestFlashArbV2 is IAaveSimpleFlashBorrower {
    ...
    /// @dev Защита от повторного входа. Не навешивается на executeOperation,
    /// т.к. Aave намеренно вызывает callback внутри startArbitrage; этот путь
    /// уже защищён проверками msg.sender == pool и loanOpen.
    ...
    function startArbitrage(
        ...
        // Авто-вывод выполняется ПОСЛЕ полного погашения займа Aave,
        // когда баланс контракта стабилен. Это исключает риск вывести
        // средства, нужные для repayment, прямо внутри flash-callback.
    ...
    function executeOperation(
        ...
        // --- NEW: учёт прибыли ---
        // Сам авто-вывод вынесен в startArbitrage и выполняется после
        // погашения займа (см. _maybeAutoWithdraw).
        if (profit > 0) {
    ...
    function _maybeAutoWithdraw(address asset) internal {
        ...
        // Уменьшаем ровно на отправленное: при cap по балансу остаток
        // учёта сохраняется и не теряется.
    ...
    function sweepToken(
        ...
        // Синхронизируем учёт прибыли: выводимые токены не должны оставаться
        // «накопленными», иначе следующий авто-вывод будет рассинхронизирован.
        uint256 acc = accumulatedProfit[token];
        if (acc > 0) {
            accumulatedProfit[token] = amount >= acc ? 0 : acc - amount;
        }
